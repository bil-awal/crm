...
const props = defineProps({
  ...
  availableSubmodules: {
    type: Array,
    default: () => [], // Ensure availableSubmodules is always an array
  },
})
...
// Define client and company data
const clientData = [
  { id: "DD2DC0A311F64EF28FD06D4C970581E1", name: "PANCARAN DARAT TRANSPORT" },
  { id: "81147DAF5C6A4B4FB47841A94AADA9F2", name: "PANCARAN ENERGI TRANSPORT" },
]
...
// Assign client/company data based on selectedType
...
// Handle client/company selection and update permissions
const handleClientCompanySelection = submodule => {
  const selectedData = submodule.clientCompanyData.find(
    item => item.id === submodule.selectedClientCompany,
  )

  if (selectedData) {
    // Set correct ID based on selectedType
    switch (submodule.selectedType) {
    case 'pdtClientId':
      submodule.pdtClientId = submodule.selectedClientCompany
      submodule.pdtCompanyId = null
      submodule.pliClientId = null
      submodule.pliCompanyId = null
      break
    case 'pdtCompanyId':
      submodule.pdtClientId = null
    ...
    if (!submodule.permissions || submodule.permissions.length === 0) {
      ...
      // Update permission name based on selected client/company
      submodule.permissions[0].name = selectedData.name
    }
  }
}

// Initialize submodule data
const initializeSubmoduleData = submodule => {
  const permission = submodule.permissions[0] || {}

  if (permission.pdtClientId) {
    submodule.selectedType = 'pdtClientId'
    submodule.selectedClientCompany = permission.pdtClientId
  } else if (permission.pdtCompanyId) {
    submodule.selectedType = 'pdtCompanyId'
    submodule.selectedClientCompany = permission.pdtCompanyId
  } else if (permission.pliClientId) {
    submodule.selectedType = 'pliClientId'
    submodule.selectedClientCompany = permission.pliClientId
  } else if (permission.pliCompanyId) {
    submodule.selectedType = 'pliCompanyId'
    submodule.selectedClientCompany = permission.pliCompanyId
  } else {
    submodule.selectedType = null
    submodule.selectedClientCompany = null
  }
...
// On mounted, initialize submodules
onMounted(() => {
  if (!submodules.value.length) {
    submodules.value = [] // Ensure submodules is always an array
  }

  submodules.value.forEach(submodule => {
    submodule.pdtClientId = submodule.pdtClientId || null
    submodule.pdtCompanyId = submodule.pdtCompanyId || null
    submodule.pliClientId = submodule.pliClientId || null
    submodule.pliCompanyId = submodule.pliCompanyId || null

    initializeSubmoduleData(submodule)
  })
})

// Deep watch submodules for changes
watch(submodules, newSubmodules => {
  const cleanedSubmodules = newSubmodules.map(({ clientCompanyData, ...rest }) => ({ ...rest }))
...
// API and Local Deletion for Submodules
const deleteSubmodule = async (submoduleId, index) => {
  if (submoduleId) {
    try {
      await $crmApi(`/submodules/${submoduleId}`, { method: 'DELETE' })
    } catch (error) {
      console.error("Failed to delete submodule via API:", error)
    }
  }
  submodules.value.splice(index, 1) // Remove locally
}

// API and Local Deletion for Role Permissions
const deleteRolePermission = async (permissionId, submoduleIndex) => {
  if (permissionId) {
    try {
      await $crmApi(`/role-permissions/${permissionId}`, { method: 'DELETE' })
    } catch (error) {
      console.error("Failed to delete role permission via API:", error)
    }
  }

  // Filter out the deleted permission
  submodules.value[submoduleIndex].permissions = submodules.value[submoduleIndex].permissions.filter(
    perm => perm.id !== permissionId,
  )
}

// Computed property to handle permissions
const getPermissionComputed = (submodule, type) => {
  return computed({
    get() {
      if (!submodule.clientCompanyData || !Array.isArray(submodule.clientCompanyData)) {
        return false
      }
      ...
      const permission = submodule.permissions?.find(
        p => p.name === clientCompanyName,
      )
    ...
    set(value) {
      ...
      let permission = submodule.permissions?.find(
        p => p.name === clientCompanyName,
      )
    ...
    >
      <VCol cols="12">
        <VAutocomplete
          ...
          @click:append-inner="() => deleteSubmodule(submodule.id, submoduleIndex)"
        />
      </VCol>
        ...
        <VRow>
          ...
          >
            <VAutocomplete
              ...
              :items="submodule.clientCompanyData"
              item-title="name"
              item-value="id"
