...
const fetchWidgetData = async () => {
  try {
    const response = await $crmApi('/dashboard/invoice-amount?menu=INVOICE_LIST_ON_REVISION')
...
const fetchInvoices = debounce(async () => {
  try {
    ...
    const url = `/invoices/on-revision?${queryString}${filtersQuery ? `&${filtersQuery}` : ''}&sort=${tableState.value.sortBy},${tableState.value.orderBy}`
...
const reviseInvoice = async () => {
  ...
  try {
    await Promise.all(selectedItems.value.map(item => 
      $crmApi(`/invoices/${item.id}/revise-action`, {
        method: 'POST',
        body: JSON.stringify({ 
          invoiceIds: [item.id],
          note: null,
