...
export const redirects = [
  {
    ...
    redirect: to => {
      ...
      const storedFeatureRoles = JSON.parse(localStorage.getItem('featureRoles')) || []
      const featureRoles = ref(storedFeatureRoles)

      watch(featureRoles, newFeatureRoles => {
        localStorage.setItem('featureRoles', JSON.stringify(newFeatureRoles))
      }, { deep: true })

      const updateFeatureRoles = newFeatureRoles => {
        featureRoles.value = newFeatureRoles
      }
      ...
      onMounted(() => {
        eventBus.on('updateUserRoles', updateFeatureRoles)
        eventBus.on('handleLogout', handleLogout)
      })

      onBeforeUnmount(() => {
        eventBus.off('updateUserRoles', updateFeatureRoles)
        eventBus.off('handleLogout', handleLogout)
      })

      if (!userData || !featureRoles.value.length) {
        return { name: 'login', query: to.query || {} }
      }

      const defaultRoute = getDefaultRoute(userData.tenantType, featureRoles.value)
      
      return { name: defaultRoute, query: to.query || {} }
    },
  },
]
